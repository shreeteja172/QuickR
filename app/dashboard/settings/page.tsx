...
export default function SettingsPage() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      toast.error("Failed to delete account. Please try again.", { id: "delete-account" });
      setDeleting(false);
    }
  };
    ...
    <main className="relative mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
      <header className="mb-10 max-w-2xl">
        <div className="mb-6 flex items-center gap-3 text-sm text-slate-500">
          <Link href="/dashboard" className="font-medium text-slate-600 hover:text-slate-950 transition">
            Dashboard
          </Link>
          <span>/</span>
        ...
        <section className="rounded-4xl border border-slate-200 bg-white p-6 shadow-[0_18px_50px_-34px_rgba(15,23,42,0.3)] sm:p-8">
          ...
          <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="space-y-3">
              ...
              />
              <p className="text-xs text-slate-500">Your email is tied to your account and cannot be changed.</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-4xl border border-slate-200 bg-white p-6 shadow-[0_18px_50px_-34px_rgba(15,23,42,0.3)] sm:p-8">
          ...
          <div className="mt-6 space-y-4">
            <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div>
                <p className="text-sm font-semibold text-slate-900">High-Resolution Downloads</p>
                <p className="text-xs text-slate-500 mt-1">Export QR codes in maximum PNG quality by default.</p>
              </div>
              <button 
                type="button"
            ...
            <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div>
                <p className="text-sm font-semibold text-slate-900">Weekly Analytics Report</p>
                <p className="text-xs text-slate-500 mt-1">Receive an email with your QR scan statistics.</p>
              </div>
              <button 
                type="button"
        ...
        <section className="rounded-4xl border border-red-200 bg-white p-6 shadow-[0_18px_50px_-34px_rgba(15,23,42,0.3)] sm:p-8">
          ...
          <div className="mt-6 flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-red-100 bg-red-50 p-5">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">Permanently remove your account and all associated QR codes.</p>
            </div>
            <button 
